  ...
  group('CategoryBadge widget', () {
    testWidgets('renders name', (tester) async {
      await tester.pumpWidget(
        _wrap(const CategoryBadge(name: 'Work', color: '#3b82f6')),
      );
    ...
    testWidgets('renders without icon when not provided', (tester) async {
      await tester.pumpWidget(
        _wrap(const CategoryBadge(name: 'Personal', color: '#ef4444')),
      );
      expect(find.textContaining('Personal'), findsOneWidget);
    });

    testWidgets('uses fallback color for invalid hex without throwing', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const CategoryBadge(name: 'Bad', color: 'not-a-color')),
      );
    ...
    testWidgets('applies parsed color to text and decoration', (tester) async {
      await tester.pumpWidget(
        _wrap(const CategoryBadge(name: 'Work', color: '#22c55e')),
      );
      ...
      expect(
        ...
        const Color(0xFF22C55E).withAlpha(84),
      );
    });

    testWidgets('trims leading icon spacing when icon is absent', (
      tester,
    ) async {
      await tester.pumpWidget(
        _wrap(const CategoryBadge(name: 'Errands', color: '#ef4444')),
      );
